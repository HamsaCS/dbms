...
def SubmitData():
    if  FIRSTNAME.get() == "" or LASTNAME.get() == "" or GENDER.get() == "" or AGE.get() == "" or ADDRESS.get() == "" or CONTACT.get() == "":
        ...
        le = leng()+1
        res = str(le)+" | "+str(FIRSTNAME.get())+" | "+str(LASTNAME.get())+" | "+str(GENDER.get())+" | "+str(AGE.get())+" | "+str(ADDRESS.get())+" | "+str(CONTACT.get())
        f = open("data.txt","a")
        f.write(res+"\n")  
        f.close()  
        fetch()
...
def UpdateData():
    if GENDER.get() == "":
       result = tkMessageBox.showwarning('', 'Please Complete The Required Field', icon="warning")
        ...
        fl = open("data.txt", "w")
        for line in data:
               if usn not in line.split(" | "):
                    fl.write(line)

        res = usn+" | "+str(FIRSTNAME.get())+" | "+str(LASTNAME.get())+" | "+str(GENDER.get())+" | "+str(AGE.get())+" | "+str(ADDRESS.get())+" | "+str(CONTACT.get())
        fl.write(res+"\n")
        f.close()
        fl.close()
        fetch()
    ...
    if not tree.selection():
       result = tkMessageBox.showwarning('', 'Please Select Something First!', icon="warning")
        ...
        if result == 'yes':
            ...
            f.close()
            fl = open("data.txt", "w")
            id = str(selecteditem[0])
            for line in data:
                if id not in line.split(" | "):
                    fl.write(line)
            fl.close()
